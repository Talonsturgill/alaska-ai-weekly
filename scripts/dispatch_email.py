...
Media is NEVER base64'd through the model. The routine uploads the cuts with
scripts/upload_video.py and passes their direct-download URLs here. This prints the draft
...
CHANNELS: the video Dispatch ships to LinkedIn (primary) + TikTok. (The separate weekly
Facebook brief is a different product — see scripts/gmail_draft.py — don't confuse the two.)

DURATION/DIMENSIONS ARE NEVER HARD-CODED. Pass the local mp4 with --video-file-vertical /
--video-file-square and the button label is probed from the actual file, so the email can
never claim a length the video doesn't have (this is what made an earlier draft say "30s"
for a 60s cut). Probing uses ffmpeg, not ffprobe (ffprobe is unreliable in some envs).

Usage:
  python scripts/dispatch_email.py \
    --post out/dispatch/post.txt \
    --video-url-vertical "<9:16 url>" --video-file-vertical out/dispatch/dispatch_9x16.mp4 \
    --video-url-square   "<4:5 url>"  --video-file-square   out/dispatch/dispatch_4x5.mp4 \
    ...
    --date 2026-06-27 --title "Cook Inlet belugas" --out-html out/dispatch/email.html
"""
import argparse, base64, json, re, shutil, subprocess, datetime as dt
...
def _ffmpeg():
    """Locate an ffmpeg binary (ffprobe is broken in some envs, so we parse ffmpeg -i)."""
...
def probe_label(path):
    """Return the '<dims> · <dur>s · H.264 MP4' button label probed from a real file.
    Falls back to just 'H.264 MP4' if the file/ffmpeg is unavailable — it NEVER invents
    a duration. This is the guard against the old hard-coded '30s' bug."""
    bits = []
    if path and Path(path).exists():
        ff = _ffmpeg()
        if ff:
            out = subprocess.run([ff, "-i", str(path)], capture_output=True, text=True).stderr
            mdim = re.search(r"Video:.*?\s(\d{2,5})x(\d{2,5})", out)
            if mdim:
                bits.append(f"{mdim.group(1)}&times;{mdim.group(2)}")
            mdur = re.search(r"Duration:\s*(\d+):(\d+):(\d+(?:\.\d+)?)", out)
            if mdur:
                total = int(mdur.group(1)) * 3600 + int(mdur.group(2)) * 60 + float(mdur.group(3))
                bits.append(f"{round(total)}s")
    bits.append("H.264 MP4")
    return " &middot; ".join(bits)

def render(post, poster_html, vids, voice, music, sources, score, note, date_str, title):
    src = "\n".join(
    ...
    buttons = ""
    if vids.get("vertical", {}).get("url"):
        buttons += (f'<a class="dl" href="{vids["vertical"]["url"]}">&#9660;&nbsp; Download &mdash; 9:16 (TikTok / full-screen)'
                    f'<small>{vids["vertical"]["label"]}</small></a>')
    if vids.get("square", {}).get("url"):
        buttons += (f'<a class="dl alt" href="{vids["square"]["url"]}">&#9660;&nbsp; Download &mdash; 4:5 (LinkedIn feed)'
                    f'<small>{vids["square"]["label"]}</small></a>')
...
def main():
    ...
    ap.add_argument("--video-url-vertical", required=True, help="9:16 TikTok cut URL")
    ap.add_argument("--video-url-square", default="", help="4:5 LinkedIn cut URL")
    ap.add_argument("--video-file-vertical", default="", help="local 9:16 mp4 — label probed from it")
    ap.add_argument("--video-file-square", default="", help="local 4:5 mp4 — label probed from it")
    ...
    vids = {
        "vertical": {"url": a.video_url_vertical, "label": probe_label(a.video_file_vertical)},
        "square":   {"url": a.video_url_square,   "label": probe_label(a.video_file_square)},
    }
    html = render(post, poster_html, vids, a.voice or "(unset)", a.music or "(unset)",
                  sources, a.score, a.note, a.date, a.title)
    if a.out_html:
