...
def main():
    ...
    # shots.json used to survive from the previous episode even after episode_props.json was
    # rebuilt. The quality gate then measured current pixels against yesterday's boundaries and
    # framings, which is a green-or-red result with no relationship to the film being shipped.
    # Derive both artifacts in the same invocation so they cannot drift independently.
    try:
        board = json.load(open(os.path.join(OUT, "storyboard.json")))
        board_shots = board.get("shots") or []
    except Exception as exc:
        raise SystemExit(f"build_scenes: could not read storyboard shots for shots.json ({exc})")
    ...
    for i, (scene, shot) in enumerate(zip(scenes, board_shots), start=1):
        gate_shots.append({"id": shot.get("id") or f"S{i}",
                           "start": scene["from"],
                           "end": scene["from"] + scene["dur"],
                           "framing": shot.get("framing", "")})
    json.dump({"fps": FPS,
               "note": f"Generated by build_scenes.py from {board.get('title', 'current storyboard')} and episode_props scene bounds.",
               "shots": gate_shots}, open(os.path.join(OUT, "shots.json"), "w"), indent=2)
