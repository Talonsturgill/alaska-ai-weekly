...
- `FullTapeMachine` transport upgrade (2026-09-03) — paper.tsx — optional `transport: 'running'|'paused'`, absolute `pausedAt` in the same frame clock as `f`; `tapeReelPhase` integrates twelve frames of decelerating braking then holds exact phase, deterministic under seeking. Omit pausedAt for initially OFF. Keep fill constant across braking; tape remains present and pause never creates FULL. Omitted transport preserves legacy fill-driven motion/FULL behavior. Optional `bodyColor`/`accentColor` restage the shelf with legacy defaults; instance-stable React ids prevent shared-coordinate gradient collisions. No literal Sully hardware claim — ACTIVE.
- `GearLever` long-throw upgrade (2026-09-03) — props.tsx — opt-in `longThrow` attaches the upward shaft and grip to pivot (-46,14), grip at (-46,-105), driven together through -40 to +40 degrees by `pulled` 0..1. Optional `accentColor` tints the grip. Default false preserves original geometry and 40-degree motion; existing optional denial badge unchanged. A schematic control, never literal patient UI — ACTIVE.
- `GripHand` (2026-09-03) — props.tsx — ordinary articulated grip hand with four wrapped digits, opposed thumb/nail, shaded wrist, ribbed cuff and continuous sleeve extending right. `x,y` locate the actual grip center; caller-eased `reach` 0..1 offsets the hand by `400*(1-reach)` world pixels to the right, independent of optional `scale`. Optional `skin`/`cuffColor`, stable instance gradient ids. Mount after the lever and compute its world grip position so contact follows the arc; no fabricated actual control UI or cultural coding — ACTIVE.
