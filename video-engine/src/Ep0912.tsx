...
const Notebook:React.FC<{x?:number;y?:number;s?:number;open?:number;wing?:number;f:number;shuffle?:number;task?:number}>=({x=540,y=880,s=1,open=1,wing=0,f,shuffle=0,task=0})=>{
 ...
 <path d="M-320-202H-28" stroke="white" strokeWidth={3} opacity={.8}/></g>
 ...
 <g transform={`translate(332 -155) rotate(${flick+1.3*Math.sin(f/21)})`}><path d="M0 0h111l13 28-13 30H0Z" fill={CORAL} stroke={INK} strokeWidth={4}/><Fit text="NEXT STEP" x={56} y={36} size={17} width={103} mono/><path d="M8 7h96" stroke="#FFE3BE" strokeWidth={3}/></g>
...
const Shot:React.FC<{n:number;from:number;dur:number;beats:Beat[]}>=({n,from,dur,beats})=>{const f=useCurrentFrame();let g=f+from;for(const id of [10,23,29]){const at=(beats.find(b=>b.id===id)?.at??999)*30,land=at+22,until=land+(id===29?21:18);if(g>=land&&g<until)g=land;}
 ...
 const push=(distance:number)=>CameraMoves.dollyThrough((200+distance*travel)/(200+distance),distance);
 const cam=n===1?composeCams(CameraMoves.orbitReveal(travel,8),push(25)):
 n===2?composeCams(CameraMoves.craneDown(.6+.4*travel,75),CameraMoves.orbitReveal(travel,4)):
 n===3?composeCams(CameraMoves.craneDown(.65+.35*travel,80),CameraMoves.truckAcross(travel,30)):
 n===4?composeCams(CameraMoves.orbitReveal(travel,5),push(20)):
 n===5?composeCams(CameraMoves.truckAcross(travel,90),CameraMoves.orbitReveal(travel,4)):
 n===6?composeCams(CameraMoves.orbitReveal(travel,6),push(22)):
 n===7?composeCams(CameraMoves.craneDown(.7+.3*travel,60),CameraMoves.truckAcross(travel,35)):
 n===8?composeCams(CameraMoves.truckAcross(travel,75),CameraMoves.orbitReveal(travel,5)):
 n===9?composeCams(CameraMoves.craneDown(.6+.4*travel,85),CameraMoves.orbitReveal(travel,8)):
 n===10?composeCams(CameraMoves.truckAcross(travel,70),CameraMoves.craneDown(.82+.18*travel,30)):
 n===11?composeCams(CameraMoves.orbitReveal(travel,10),push(20)):
 n===12?composeCams(CameraMoves.truckAcross(travel,80),CameraMoves.orbitReveal(travel,4)):
 n===13?composeCams(CameraMoves.craneDown(.7+.3*travel,55),push(20)):
 composeCams(CameraMoves.orbitReveal(travel,4),push(30));
 ...
 {room==='archive'?<g>{[0,1,2].map(i=><g key={i}><rect x={70} y={650+i*160} width={930} height={130} rx={8} fill={GROUND} stroke={INK} strokeWidth={6}/>{[0,1,2,3,4,5].map(k=><g key={k}><rect x={93+k*148} y={663+i*160} width={126} height={91} rx={6} fill={PAPER} stroke={INK} strokeWidth={4}/><path d={`M${135+k*148} ${710+i*160}h43`} stroke={CORAL} strokeWidth={8}/></g>)}</g>)}</g>:<g><path d="M125 775V520h72" fill="none" stroke={INK} strokeWidth={9}/><g transform={`translate(202 543) rotate(${quiet?0:2*Math.sin(g/31)})`}><rect x={-48} y={-25} width={96} height={50} rx={8} fill={CORAL} stroke={INK} strokeWidth={4}/><path d="M-25 0h50" stroke={PAPER} strokeWidth={5}/></g><path d={`M920 620q${18+2*Math.sin(g/29)} 35 0 70t0 70`} fill="none" stroke={INK} strokeWidth={5}/><path d={`M170 1190q55 ${9+Math.sin(g/35)*3} 82 0`} fill="none" stroke={CORAL} strokeWidth={8}/></g>}
