...
EVENTS = [
    (4.0,  "whoosh",   "whoosh", "descent whoosh into the valley"),
    (7.5,  "tick",     "tick",   "forecast grid UI tick"),
    (11.0, "whoosh",   "whoosh", "swell on the cut to the cross-section"),
    (14.6, "boom",     "boom",   "low boom on the 5x undercount"),
    ...
    (44.2, "tick",     "tick",   "HUD refocus tick, reality unchanged"),
    (48.2, "pulse",    "pulse",  "searching ping returns empty"),
    (52.2, "riser",    "riser",  "airy lift and confirming chime"),
    (55.6, "hit",      "hit",    "soft brand sting"),
]
...
# ---------------- ambient bed: low wind + distant fire crackle ----------------
tt = t(TOTAL)
wind = lp(rng.standard_normal(N), 300) * (0.55 + 0.45 * np.sin(2 * np.pi * 0.05 * tt))
crackle = np.zeros(N, np.float32)
for _ in range(90):
    st = rng.uniform(0, 59); n = int(rng.uniform(.01, .03) * SR); i = int(st * SR)
    crackle[i:i + n] += bp(rng.standard_normal(n), 1500, 5000) * np.exp(-np.linspace(0, 1, n) * 5) * rng.uniform(.2, .6)
amb = wind * 0.7 + crackle * 0.5
